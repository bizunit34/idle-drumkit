# QA Run Template

Copy this into a GitHub QA finding or test report when running internal alpha checks.

## Run Details

- Tester:
- Date:
- App version / build number:
- Device:
- OS version:
- Install method:
- Orientation tested: Portrait / Landscape / Both
- Screens tested: Home / Drum Set / MIDI Controller / Settings
- Carousel tested: Swipe / Press / Dots / Not tested
- Start overlay tested: Yes / No
- MIDI edit tested: Performance mode / Edit Pads mode / Pad Edit / Not tested
- MIDI playback behavior tested: One-shot / Gate / Toggle / Choke / Not tested
- Drum edit tested: Select / Drag / Resize item / Resize hit box / Resize both / Resize target toolbar / Orientation layouts / Not tested
- External music tested: Yes / No

## Result

- Test case:
- Result: Pass / Fail / Blocked
- Severity: Low / Medium / High / Blocking
- Screen:
- Articulation involved, if any:
- Notes:
- Screenshot / video link:

## Steps

1.
2.
3.

## Expected Result

## Actual Result
